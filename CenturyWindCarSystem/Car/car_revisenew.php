...
if(@$_POST['owner']!=null)
{
    $query="update cars set plateNumber='".$a['plateNumber']."',type='".$a['type']."',brand='".$a['brand']."',owner='".$a['owner']."',ownerType='".$a['ownerType']."',phone='".$a['phone']."'
...
if(@$_POST['owner']!=null)
{
    ...
    {
        $query="update into car_owner_personal set plateNumber='".$a['plateNumber']."',name='".$a['op_name']."',sex='".$a['op_sex']."',nation='".$a['op_nation'].
            "',birthday='" .$a['op_birthday']."',address='" .$a['op_address']."',number='".$a['op_number']."',organization='".$a['op_organization']."',usefulDate='".$a['op_usefulDate']."',phone='".$a['phone']."'
            ";
        $s=mysql_query($query) or die("Error in query: $query. ".mysql_error());
        ++$s;
    ...
    {
        $query="update into car_owner_unit set plateNumber='".$a['plateNumber']."',socialCreditCode='".$a['ou_socialCreditCode']."',name='".$a['ou_name']."',type='".$a['ou_type'].
            "',address='".$a['ou_address']."',legalRepresentative='".$a['ou_legalRepresentative']."',registeredCapital='" .$a['ou_registeredCapital']."',setUpDate='" .$a['ou_setUpDate'].
            "',businessUsefulDate='".$a['ou_businessUsefulDate']."',businessScope='".$a['ou_businessScope']."',organization='".$a['ou_organization']."',releaseDate='".$a['ou_releaseDate']."',phone='".$a['phone']."'
            ";
        $s=mysql_query($query) or die("Error in query: $query. ".mysql_error());
        ++$s;
...
{
    $query="update into car_info set plateNumber='".$a['plateNumber']."',brandModel='".$a['i_brandModel']."',vehicleCode='".$a['i_vehicleCode']."',engineNumber='".$a['i_engineNumber'].
        "',passenger='".$a['i_passenger']."',color='".$a['i_color']."'
        ";
    $s=mysql_query($query) or die("Error in query: $query. ".mysql_error());
    ++$s;
...
{
    $query="update car_insurance set plateNumber='".$a['plateNumber']."',name='".$a['ie_name']."',company='".$a['ie_company']."',number='".$a['ie_number']."',amount='".$a['ie_amount'].
...
{
    $query="update car_regist set plateNumber='".$a['r_plateNumber']."',type='".$a['r_type']."',owner='".$a['r_owner']."',address='".$a['r_address']."',organization='".$a['r_organization'].
        "',registDate='" .$a['r_registDate']."',usefulDate='".$a['r_usefulDate']."',registUnit='".$a['registUnit']."',registPerson='".$a['registPerson']."',costName='".$a['costName']."',costAmount='".$a['costAmount']."'
        ";
    $s=mysql_query($query) or die("Error in query: $query. ".mysql_error());
    ++$s;
...
<div class="col8">
	<div id="checkNum">
		<table class="table-bordered table" style="text-align: center">
			<tr>
				<td><p><input id="checkall" type="checkbox" onchange="showModel();"/>选中全部</p></td>
			...
			<tr>
				<td></td>
				<td><p><input id="checkfive" type="checkbox" checked="checked" onchange="showModel();"/>注册信息</p></td>
...
<div id="mineInfo" class="col8">
    ...
    <table class="table table-bordered">
        ...
        <tr>
            <td>物品种类</td>
            <td><input type="text" name="p_class" placeholder="救援设备"/></td>
        </tr>
        </tbody>
        <tbody>
        <tr>
            <td>物品类别</td>
            <td><input type="text" name="p_type" placeholder="液压千斤顶"/></td>
        </tr>
        <tr>
            <td>物品品牌</td>
            <td><input type="text" name="p_brand" placeholder=""/></td>
        </tr>
        <tr>
            <td>物品编号</td>
            <td><input type="text" name="p_number" placeholder="-"/></td>
        </tr>
        <tr>
            <td>销售单位</td>
            <td><input type="text" name="p_saleUnit" placeholder="随州厦工楚胜专用汽车有限公司"/></td>
        </tr>
        <tr>
            <td>销售人员</td>
            <td><input type="text" name="p_salePerson" placeholder="李夏功"/></td>
        </tr>
        <tr>
            <td>联系电话</td>
            <td><input type="text" name="p_phone" placeholder="18190686696"/></td>
        </tr>
        <tr>
            <td>厂商指导价</td>
            <td><input type="text" name="p_factoryPrice" placeholder="187000"/></td>
        </tr>
        <tr>
            <td>采购价格</td>
            <td><input type="text" name="p_purchasePrice" placeholder="145800"/></td>
        </tr>
        <tr>
            <td>购买方式</td>
            <td><input type="text" name="p_buyWay" placeholder="贷款按揭"/></td>
        </tr>
        <tr>
            <td>采购人员</td>
            <td><input type="text" name="p_buy_person" placeholder="刘敏"/></td>
        </tr>
        <tr>
            <td>财务审核人</td>
            <td><input type="text" name="p_financialPerson" placeholder=""/></td>
        </tr>
        <tr>
            <td>审核意见</td>
            <td><input type="text" name="p_financialOpinion" placeholder=""/></td>
        </tr>
        <tr>
            <td>审核时间</td>
            <td><input type="text" name="p_financialDate" placeholder=""/></td>
        </tr>
        <tr>
            <td>经理审核人</td>
            <td><input type="text" name="p_manager" placeholder=""/></td>
        </tr>
        <tr>
            <td>审核意见</td>
            <td><input type="text" name="p_managerOpinion" placeholder=""/></td>
        </tr>
        <tr>
            <td>审核时间</td>
            <td><input type="text" name="p_managerDate" placeholder=""/></td>
        </tr>
        </tbody>
    </table>
    </p>
...
//					alert(opt.value+"io");
    ...
    function showModel(){
	    if(document.getElementById("checkall").checked){
		    ...
		    document.getElementById("checkthree").checked=true;
		    document.getElementById("checkfour").checked=true;
		    document.getElementById("checkfive").checked=true;
		    document.getElementById("checksix").checked=true;
		    document.getElementById("checkseven").checked=true;
		    document.getElementById("checkeight").checked=true;
	    }
