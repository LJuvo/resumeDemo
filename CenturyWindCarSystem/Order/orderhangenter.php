			...
			<div class="row">
				...
				<div class="col9 docs-body" id="main_content">
					<table class="table table-bordered" style="text-align:center" >
						<tr>
							<td><a href="orderhang.php">待派订单</a></td>
							<td class="selected"><a href="#">待确认订单</a></td>
					...
					<table class="table table-bordered" style="text-align:center" id="addNewTable">
						...
						$query="SELECT * FROM order_employer_state WHERE employer_name='$userName' AND employer_state='待确认'";
						...
						if($temp>0)
						{
							for($i=0;$i<=$temp-1;$i++)
							{
								...
								<td>$rocc[3]</td><td>$row[1]</td><td><a href='enterorder.php?employer_tel=$row[2]&Id=$row[6]' class='btn bg-inverse bg-blue'>确认订单</button></td></tr>";
							}
						}
					...
					<table class="table table-bordered" style="text-align:center" id="addNewTable">
						...
						$query="SELECT * FROM order_employer_state WHERE employer_state='待确认'";
						$result = mysql_query($query) or die("Error in query: $query. ".mysql_error());
						$temp=mysql_num_rows($result);
						if($temp>0)
						{
							for($i=0;$i<=$temp-1;$i++)
							{
								...
								echo"<tr><td>$row[6]</td><td>$rocc[7]</td><td>$rocc[1]</td>
								<td>$rocc[3]</td><td>$row[1]</td><td>$row[3]</td></tr>";
							}
						}
